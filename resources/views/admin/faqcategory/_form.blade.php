    ...
    @foreach ($langs as $lable => $lang)
        ...
        <div id="{{ $lable }}" class="tab-pane fade in {{ $i == 1 ? 'active' : '' }}">
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group {{ $errors->first($lable . '.name') ? 'has-error' : '' }}">
                            <label for="faq_category_{{ $lable }}_name">{{ __('Name') }}</label> :<span
                                class="require">*</span>
                            {!! Form::text($lable . '[name]', $result != '' ? $result->translate($lable)->name : null, ['class' => 'form-control required faqcategory', 'id' => 'faq_category_' . $lable . '_name']) !!}
                    ...
                    <div class="col-sm-6">
                        <div
                            ...
                            <label
                                for="faq_category_{{ $lable }}_short_description">{{ __('Short description') }}</label>
                            :
                            {!! Form::textarea($lable . '[short_description]', $result != '' ? $result->translate($lable)->short_description : null, ['class' => 'form-control', 'rows' => 4, 'cols' => 30, 'id' => 'faq_category_' . $lable . '_short_description']) !!}
            ...
            <div class="col-sm-4 border-left">
                @include('admin.general.titleAndSlug', array('lable' => $lable, 'class_name' => 'faqcategory',
                'model_name'=>'faq_category_', 'result'=>$result))
            </div>
            <div class="clearfix"></div>
        </div>
        @php
    ...
    <div id="general" class="tab-pane fade">
        <div class="col-sm-2">
            <div class="form-group">
                <label class="control control--checkbox">
                    {{ Form::checkbox('status', 1, $result != '' ? $result->status : true, ['id' => 'faq_category_status']) }}
                    {{ __('Status') }}
        ...
        @if ($result != '')
            <div class="col-sm-4">
                ...
                array(
                'size' =>(config('imageSize.FaqCategory.big') ? config('imageSize.FaqCategory.big') : 0),// $size,
                'page_id'=>$result['id'],
